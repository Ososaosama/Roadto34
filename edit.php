...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['updateTour'])) {
    ...
    try {
        ...
        $updateQuery = $pdo->prepare("
            UPDATE tour SET guideId = ?, location = ?, date = ?, price = ?, imageURL = ?, description = ?, city = ?, duration = ?, title = ?, placeCategory = ? 
            WHERE tourId = ?
        ");
        $updateQuery->execute([$guideId, $location, $date, $price, $imagePath, $description, $city, $duration, $title, $placeCategory, $tourId]);
...
<div class="container mt-5">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            <label for="placeCategory" class="form-label">التصنيف</label>
            <input type="text" class="form-control" id="placeCategory" name="placeCategory" value="<?= htmlspecialchars($currentTour['placeCategory']) ?>" required>
        </div>
