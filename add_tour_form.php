...
if (!isset($_SESSION['userId']) || $_SESSION['role'] !== 'guide') {
    echo "خطأ: لم يتم العثور على هوية المستخدم أو ليس لديك الصلاحية. يرجى تسجيل الدخول.";
    exit;
}
...
<div class="container mt-5">
    ...
    <h1 class="mb-4" style="color:#000;">إضافة جولة</h1>
    <form method="POST" enctype="multipart/form-data">
        <input type="hidden" name="addTour" value="1">

        <div class="mb-3">
            <label for="guideId" class="form-label" style="color:#FFF;">المرشد</label>
            <input type="text" class="form-control" id="guideId" name="guideId" value="<?= htmlspecialchars($guide['name']) ?>" readonly>
        </div>
        <div class="mb-3">
            <label for="location" class="form-label" style="color:#FFF;">موقع الجولة</label>
            <input type="text" class="form-control" id="location" name="location" required>
        </div>
        <div class="mb-3">
            <label for="date" class="form-label" style="color:#FFF;">التاريخ</label>
            <input type="date" class="form-control" id="date" name="date" required>
        </div>
        <div class="mb-3">
            <label for="price" class="form-label" style="color:#FFF;">السعر</label>
            <input type="number" step="0.01" class="form-control" id="price" name="price" required>
        </div>
        <div class="mb-3">
            <label for="title" class="form-label" style="color:#FFF;">العنوان</label>
            <input type="text" class="form-control" id="title" name="title" required>
        </div>
        <div class="mb-3">
            <label for="description" class="form-label" style="color:#FFF;">الوصف</label>
            <textarea class="form-control" id="description" name="description" required></textarea>
        </div>
        <div class="mb-3">
            <label for="city" class="form-label" style="color:#FFF;">اسم المسار</label>
            <input type="text" class="form-control" id="city" name="city" required>
        </div>
        <div class="mb-3">
            <label for="duration" class="form-label" style="color:#FFF;">المدة (بالساعات)</label>
            <input type="number" class="form-control" id="duration" name="duration" required>
        </div>
        <div class="mb-3">
            <label for="tourImage" class="form-label" style="color:#FFF;">صورة الجولة (اختياري)</label>
            <input type="file" class="form-control" id="tourImage" name="tourImage" accept="image/*">
        </div>
        <div class="mb-3">
            <label for="placeCategory" class="form-label" style="color:#FFF;">التصنيف</label>
            <input type="text" class="form-control" id="placeCategory" name="placeCategory" required>
        </div>
        <div class="mb-3">
            <label for="startDate" class="form-label" style="color:#FFF;">تاريخ من</label>
            <input type="date" class="form-control" id="startDate" name="startDate" required>
        </div>
        <div class="mb-3">
            <label for="endDate" class="form-label" style="color:#FFF;">تاريخ إلى</label>
            <input type="date" class="form-control" id="endDate" name="endDate" required>
        </div>
        
        <div class="mb-3">
            <label style="color:#FFF;">اختر الأماكن</label>
            <div>
        ...
        <button type="submit" class="btn btn-primary" style="border: 2px solid #000;">حفظ الجولة</button>
    </form>

    <h1 class="mt-5" style="color:#000;">جميع الجولات الخاصة بالمرشد</h1>
    <table class="table table-bordered">
        <thead>
            <tr style="background-color: #087880; color: #FFF;">
                <th>العنوان</th>
                <th>الموقع</th>
                <th>التاريخ</th>
                <th>السعر</th>
